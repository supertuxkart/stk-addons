...
-- Procedures
--
CREATE DEFINER=`root`@`%` PROCEDURE `v2_create_file_record`(IN id TEXT, IN atype TEXT, IN ftype TEXT, IN fname TEXT, OUT insertid INT)
BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `v2_increment_download`(IN filepath TEXT)
UPDATE `v2_files`
    SET `downloads` = `downloads` + 1
    WHERE `file_path` = filepath$$

CREATE DEFINER=`root`@`%` PROCEDURE `v2_log_event`(IN in_user INT(10) unsigned, IN in_message TEXT)
INSERT INTO `v2_logs`
    (`user`,`message`)
    VALUES
    (in_user,in_message)$$

CREATE DEFINER=`root`@`%` PROCEDURE `v2_register_user`(IN in_user TEXT, IN in_pass CHAR(96), IN in_name TEXT, IN in_email TEXT, IN in_vercode TEXT, IN in_regdate DATE)
...
CREATE DEFINER=`root`@`%` PROCEDURE `v2_set_logintime`(IN userid INT(11), IN logintime TIMESTAMP)
UPDATE `v2_users`
