    ...
    function __toString() {
        ...
        try {
            if (User::$logged_in) {
                ...
                if ($_SESSION['role']['manageaddons'] == true || $this->addon->getUploader() == User::$user_id) {
    ...
    public function fillTemplate($template) {
        ...
        foreach ($revisions AS $rev_n => $revision) {
            if (!User::$logged_in) {
                ...
                if (($this->addon->getUploader() != $_SESSION['userid'] && !$_SESSION['role']['manageaddons']) && !($revision['status'] & F_APPROVED))
                    continue;
            }
            $rev = array(
                'number' => $rev_n,
        ...
        foreach ($image_files_db AS $image) {
            ...
            if (User::$logged_in) {
                if ($_SESSION['role']['manageaddons']) {
                    if ($image['approved'] == 1)
                        $admin_links .= '<a href="' . File::rewrite($this->addon->getLink() . '&amp;save=unapprove&amp;id=' . $image['id']) . '">' . htmlspecialchars(_('Unapprove')) . '</a>';
                    else
                        $admin_links .= '<a href="' . File::rewrite($this->addon->getLink() . '&amp;save=approve&amp;id=' . $image['id']) . '">' . htmlspecialchars(_('Approve')) . '</a>';
                    $admin_links .= '<br />';
                }
                if ($_SESSION['role']['manageaddons'] || $this->addon->getUploader() == $_SESSION['userid']) {
                    if ($this->addon->getType() == 'karts') {
                        if ($this->addon->getImage(true) != $image['id']) {
                            $admin_links .= '<a href="' . File::rewrite($this->addon->getLink() . '&amp;save=seticon&amp;id=' . $image['id']) . '">' . htmlspecialchars(_('Set Icon')) . '</a><br />';
                        }
                    }
                    if ($this->addon->getImage() != $image['id']) {
                        $admin_links .= '<a href="' . File::rewrite($this->addon->getLink() . '&amp;save=setimage&amp;id=' . $image['id']) . '">' . htmlspecialchars(_('Set Image')) . '</a><br />';
                    }
                    $admin_links .= '<a href="' . File::rewrite($this->addon->getLink() . '&amp;save=deletefile&amp;id=' . $image['id']) . '">' . htmlspecialchars(_('Delete File')) . '</a><br />';
                }
            }
        ...
        foreach ($source_files_db AS $source) {
            ...
            $source['details'] .= '<a href="' . DOWN_LOCATION . $source['file_path'] . '" rel="nofollow">' . htmlspecialchars(_('Download')) . '</a>';
            if (User::$logged_in) {
                if ($_SESSION['role']['manageaddons']) {
                    if ($source['approved'] == 1)
                        $source['details'] .= ' | <a href="' . File::rewrite($this->addon->getLink() . '&amp;save=unapprove&amp;id=' . $source['id']) . '">' . htmlspecialchars(_('Unapprove')) . '</a>';
                    else
                        $source['details'] .= ' | <a href="' . File::rewrite($this->addon->getLink() . '&amp;save=approve&amp;id=' . $source['id']) . '">' . htmlspecialchars(_('Approve')) . '</a>';
                }
                if ($this->addon->getUploader() == $_SESSION['userid'] || $_SESSION['role']['manageaddons'])
                    $source['details'] .= ' | <a href="' . File::rewrite($this->addon->getLink() . '&amp;save=deletefile&amp;id=' . $source['id']) . '">' . htmlspecialchars(_('Delete File')) . '</a><br />';
            }
            if (User::$logged_in &&
                    ($this->addon->getUploader() == $_SESSION['userid'] || $_SESSION['role']['manageaddons'])) {
                $source_files[] = $source;
