...
class User extends Base implements IAsXML
{
    ...
    /**
     * Get nth top player based on ranking scores (called by everyone)
     *
     * @param int $ntop
     *
     * @return list of a player info with rank
     */
    public static function getTopPlayersFromRanking($ntop)
    {
        try
        {
    ...
    public static function submitRanking($permission, $id_for_ranked, $new_scores, $new_max_scores, $new_num_races_done)
    {
        ...
        {
            DBConnection::get()->query(
                "REPLACE INTO `{DB_VERSION}_rankings` (id, username, scores,
                max_scores, num_races_done) VALUES (:id, :username, :scores, :max_scores, :num_races_done)",
                DBConnection::NOTHING,
                [
                    ':id' => $id_for_ranked,
                    ':username' => User::getFromID($id_for_ranked)->getUserName(),
                    ':scores' => $new_scores,
                ...
                [
                    ':id' => DBConnection::PARAM_INT,
                    ':username' => DBConnection::PARAM_STR,
