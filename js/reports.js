...
function drawCharts() {
	...
	$.ajax({
	    ...
	    error: function() {document.ready = true;},
	    success: function(data, a, b){ document.ready = true; drawPie(data, Element); }
	});
    });
	...
	$.when( $.ajax({
	    ...
	    jsonp: false,
	    jsonpCallback: graph_id,
	    error: function() {$('.time_chart').index(index).innerHTML = 'Failed!'},
	    success: function(data, a, jqXHR){drawTimeChart(data, Element); }
	}) ).then(function(){});
    });
}
